...
def initialize(root, campaign, target=None, python=None):
    ...
    if not path.exists():
        save(path, {'kind': 'fine-annotation-campaign-v1', 'createdAt': now(),
                    ...
                    'python': python or str(REPO / '.local/annotation-harness-venv/bin/python'), 'batches': []})
    config = read(path)
...
def prepare_batch(root, config, batch):
    ...
    if not snapshot.exists():
        subprocess.run([config['python'], str(REPO / 'scripts/annotation-priorities.py'),
    ...
    if not bundle.exists():
        subprocess.run([config['python'], str(REPO / 'scripts/prepare-annotation-harness.py'),
