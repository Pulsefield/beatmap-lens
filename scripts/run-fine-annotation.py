...
base = module('run-section-benchmark')
preparer = module('prepare-harness-benchmark')
...
def freeze_skill(root):
    source = REPO / 'skills/mania-pattern-judgment'
...
def prepare(root, queue_path, bundle, campaign, python, max_sections=5, max_brief_chars=28000):
    ...
    for role in ('labeler', 'auditor'):
        shutil.copyfile(REPO / 'docs/agent-roles' / ('harness-' + role + '.md'), root / 'common' / (role + '.md'))
