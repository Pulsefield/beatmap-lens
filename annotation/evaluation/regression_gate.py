...
def source_snapshot(repo=REPO, ref=None):
    ...
    suite = json.loads(git_bytes(repo, ref, SUITE)) if ref else read(repo / SUITE)
    return {'files': files, 'suiteSha256': digest(suite)}
...
def changed_roles(baseline, candidate):
    ...
    if baseline['suiteSha256'] != candidate['suiteSha256']:
        roles.append('corpus')
    return roles
...
def compare(evidence, suite, baseline, candidate):
    ...
    errors, regressions = [], []
    if evidence.get('kind') != 'annotation-regression-evidence-v1':
...
def check(repo=REPO, evidence_path=None, base_ref=None):
    repo = Path(repo)
    ...
    path = Path(evidence_path) if evidence_path else repo / EVIDENCE
    if not path.exists():
        return {'passed': False, 'changedCoverage': roles,
                'errors': ['Annotation sources changed without fresh baseline/candidate regression evidence.']}
    return {'changedCoverage': roles, **compare(read(path), read(repo / SUITE), baseline, candidate)}
...
def collect(baseline_root, candidate_root, review_path, out):
    ...
    result = {'kind': 'annotation-regression-evidence-v1', 'baseline': baseline, 'candidate': candidate,
              'suiteSha256': digest(suite), 'runs': {'baseline': old, 'candidate': new}}
...
def main():
    ...
    try:
        result = check(evidence_path=args.evidence, base_ref=args.base_ref) if args.command == 'check' else \
