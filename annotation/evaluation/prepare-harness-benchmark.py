...
extract_examples, public_example = examples.extract_examples, examples.public_example

ARMS = ('fixed-evidence', 'harness-a', 'harness-b')


def module(name):
    directory = REPO / 'harness' if name == 'prepare-annotation-harness' else Path(__file__).parent
    return load_module(directory / (name + '.py'))
...
def prepare(design_path, root, campaign, python):
    ...
    skill_root = REPO / '.agents/skills/mania-pattern-judgment'
...
'''
    for arm in ARMS:
        ...
        if uses_harness:
            (job / 'ROLE.md').write_bytes((REPO / 'annotation/roles/harness-labeler.md').read_bytes())
