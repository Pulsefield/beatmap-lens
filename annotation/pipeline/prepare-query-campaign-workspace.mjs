...
async function main() {
  ...
  if (values.help) {
    process.stdout.write(
      "Prepare a proposed Foundation without canonical writes:\n  node annotation/pipeline/prepare-query-campaign-workspace.mjs --old-workspace OLD --workspace NEW --out PREPARED.json\n\nApprove the reviewed artifact and register its first task (no workers or services):\n  node annotation/pipeline/prepare-query-campaign-workspace.mjs --approve --prepared PREPARED.json --prepared-sha SHA --human-id ID --authorization TEXT\n",
    );
    return;
  }
  if (values.approve) {
