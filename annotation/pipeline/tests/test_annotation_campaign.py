...
spec = importlib.util.spec_from_file_location(
    "campaign", Path(__file__).resolve().parent.parent.joinpath("run-annotation-campaign.py"))
...
class CampaignStopTest(unittest.TestCase):
    def test_user_stop_blocks_run_before_setup_and_keeps_status_available(self):
        with TemporaryDirectory() as temporary:
            ...
            command = [sys.executable, str(Path(__file__).resolve().parent.parent.joinpath("run-annotation-campaign.py"))]
...
class QueryFirstJobTest(unittest.TestCase):
    def setUp(self):
        ...
        for name in campaign.QUERY_TOOLS:
            (self.common / name).write_bytes((campaign.REPO / ("annotation/pipeline" if name in ("check-annotation-result.py", "prepare-query-evidence.py") else "harness") / name).read_bytes())
    ...
    def test_duration_and_concurrency_caps_apply_to_new_campaign(self):
        ...
        result = subprocess.run([sys.executable, str(campaign.REPO / "annotation/pipeline/run-annotation-campaign.py"),
    ...
    def test_evidence_only_assignment_can_prepare_a_revision_without_auditor_parquet(self):
        ...
        spec = importlib.util.spec_from_file_location("revision", campaign.REPO / "annotation/pipeline/prepare-annotation-revision.py")
...
class RecoveryFoundationViewTest(unittest.TestCase):
    def test_projected_foundation_requires_exact_common_pin_and_exact_derived_definitions(self):
        spec = importlib.util.spec_from_file_location("recovery", Path(__file__).resolve().parent.parent.joinpath("prepare-annotation-recovery.py"))
...
class NecessaryConditionRuleTest(unittest.TestCase):
    def test_complete_source_and_half_open_distinct_column_occupancy(self):
        spec = importlib.util.spec_from_file_location("query_evidence", Path(__file__).resolve().parent.parent.joinpath("prepare-query-evidence.py"))
