...
SCRIPT = Path(__file__).resolve().parent.parent.joinpath("check-annotation-result.py")
REPO = SCRIPT.resolve().parents[2]
spec = importlib.util.spec_from_file_location("result_check", SCRIPT)
checker = importlib.util.module_from_spec(spec)
spec.loader.exec_module(checker)
CASES = json.loads((SCRIPT.resolve().parents[2] / "scripts/fixtures").joinpath("pattern-queries-reviewed.json").read_text())["cases"]
